	...
	<view class="page">
		...
		<view class="login-box">
			...
			<view class="login-card-input">
				<!-- 输入框头部图标 -->
				<uni-easyinput class="hj-input" v-model="value" focus maxlength="30" :trim="true"
				...
				<!-- 输入框头部图标 -->
				<uni-easyinput class="hj-input" v-model="value" type="password" @focus="PassWordFocusBorderStyle"
					@blur="PassWordBlurBorderStyle" :styles="inputColorConfigToPassWord" placeholder="请输入密码">
				</uni-easyinput>
			</view>
			<view class="login-card-loginIn">
				<view class="login-card-loginIn-btn" hover-class="login-card-loginIn-btn-hover">
					登 录
				</view>
			</view>
	...
	* {
	...
	.login-box {
		...
		.login-card-loginIn {
			...
			.login-card-loginIn-btn-hover {
				background-color: #45d4b0;
				color: white;
			}
		}
